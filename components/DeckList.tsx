...
import toast from 'react-hot-toast'; // <-- 2. Import toast สำหรับแจ้งเตือน
...
const CardCategory = ({ title, cards, onRemoveCard, getCardCount }: {
  ...
  return (
    <div>
      <h3 className="text-sm font-bold text-text-main bg-dark-bg/50 px-3 py-1.5 sticky top-0 z-10">{title} ({totalInCategory})</h3>
      <ul>{sortedCards.map(card => (
        <li key={card.RuleName} className="flex justify-between items-center px-3 py-2 border-b border-dark-border hover:bg-dark-border/50">
          <div className="flex items-center min-w-0">
            <div className="flex-shrink-0">{card.Image && <Image src={card.Image} alt={card.Name} width={30} height={42} className="rounded-sm" />}</div>
            <span className={`text-sm truncate ${card.Image ? 'ml-3' : ''}`}>{card.Name}</span>
          </div>
          <div className="flex items-center gap-3 flex-shrink-0">
            <span className="text-sm font-bold w-6 text-center">x{getCardCount(card.RuleName)}</span>
            <button onClick={() => onRemoveCard(card.RuleName)} className="bg-card-border text-white rounded-full w-5 h-5 flex items-center justify-center text-xs hover:opacity-80 transition-opacity">-</button>
          </div>
        </li>
      ))}</ul>
    </div>
...
export default function DeckList({ deckCards, onRemoveCard, getCardCount, onClearDeck }: {
  ...
  // --- ✨ 3. แก้ไข `handleSavePdf` ให้เป็น async ---
  const handleSavePdf = async () => {
    if (!isDeckValid) {
      toast.error("Your deck is not valid. Please fix the errors first.");
      return;
    }
    toast.loading('Generating PDF...'); // แจ้งเตือนว่ากำลังสร้าง
    toast.dismiss(); // ปิด loading toast
    ...
    <div className="bg-dark-surface h-full flex flex-col">
      <header className="p-4">
        <div className="flex justify-between items-center mb-2">
          ...
          {deckCards.length > 0 && <button onClick={onClearDeck} className="text-xs text-text-muted hover:text-card-border">Clear All</button>}
        </div>
      ...
      <footer className="p-4 border-t border-dark-border mt-auto bg-dark-bg">
        ...
        <div className="grid grid-cols-2 gap-2 mt-4">
          <Link href="/view" legacyBehavior><a className={`w-full text-center block p-2 rounded-md font-bold transition-colors ${deckCards.length > 0 ? 'bg-gray-600 hover:bg-gray-500 text-white' : 'bg-dark-border text-text-muted cursor-not-allowed'}`} onClick={(e) => deckCards.length === 0 && e.preventDefault()}>View Deck</a></Link>
          
          {/* --- ✨ 4. เปลี่ยนจาก <Link> มาเป็น <button> ที่ถูกต้อง --- */}
          <button
            onClick={handleSavePdf}
            disabled={!isDeckValid}
            className={`w-full text-center block p-2 rounded-md font-bold transition-colors ${isDeckValid ? 'bg-blue-600 hover:bg-blue-500 text-white' : 'bg-dark-border text-text-muted cursor-not-allowed'}`}
          >
            Save as PDF
          </button>
        </div>
